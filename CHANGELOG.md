...
- `aimock-pytest`: `AIMockServer.reset()` and `.reset_fixtures()` now call `POST /__aimock/reset`. Both already performed a full reset; the observable behavior is unchanged and neither emits a deprecation warning any more.
