...
3. **PR** — the workflow always opens a pull request that a human reviews + merges (no auto-merge). There are two distinct PR classes:
   ...
   **You can edit these PR bodies freely — the markers repair themselves.** Both `<!-- drift-* -->` markers are machine state living inside prose a human owns, and on 2026-08-03 a wholesale body rewrite of PR #343 deleted them; ~14h later the scheduled run could not find the changeset key and opened duplicate PR #350. So before deduping, each PR-open step now **re-asserts its own markers**: it lists its open PRs by state a human cannot edit — the head branch ends in `-<changeset key>`, plus (for the needs-human path) the note paths in the PR's own diff — appends back whichever markers went missing, and logs a `::warning::`. A body that still has all its markers is left byte-identical, so the repair is a no-op on a healthy PR and never duplicates a marker. Deleting a marker therefore costs you one warning line in the next run's log, not a duplicate PR.
