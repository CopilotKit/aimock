...
describe("fix-drift.yml — an unusable provider credential cannot read as 'no drift'", () => {
   ...
   * half when the live listing is shorter than the number of families aimock mocks
   * for that provider (a partial response, or an API that changed shape). Reading
   * the reason off the real call is what makes this fixture unable to drift from
   * the emitter — the previous fixture in this describe hand-copied log prose and
